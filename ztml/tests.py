                    ...
                    for mtf in mtf_variants:
                        for bitdepth in bitdepths:
                            for render_mode in range(3):
                                ...
                                i += 1
                                print(f'{i}/{len(browsers) * len(input_encodings) * len(bin2txt_encodings) * len(caps_modes) * len(mtf_variants) * len(bitdepths) * 3} browser={browser} input_enc={encoding} bin2txt={bin2txt} caps={caps} mtf={mtf} bitdepth={bitdepth} id={bool(element_id)} raw={raw}')
                                suffix = f'{browser}_{encoding}_{bin2txt}_{caps}_{mtf}_{bitdepth}'
                                if element_id:
                                    suffix += '_id'
                                if raw:
                                    suffix += '_raw'
                                ...
                                if encoding.replace('-', '') == 'utf8':
                                    ...
                                    out1, result1 = ztml.ztml(text, unix_newline=False, caps=caps, mtf=mtf, bitdepth=bitdepth, bin2txt=bin2txt, element_id=element_id, raw=raw, validate=True, browser=b, verbose=True)
                                    out2, result2 = ztml.ztml(text, output_filename, unix_newline=False, caps=caps, mtf=mtf, bitdepth=bitdepth, bin2txt=bin2txt, element_id=element_id, raw=raw, validate=True, browser=b, verbose=True)
                                ...
                                result1 = os.system(f'python ztml.py "{input_filename}" "{output_filename}" --skip_unix_newline --caps {caps} --mtf {mtf} --bitdepth {bitdepth} --bin2txt {bin2txt} {element_id_or_raw_arg} --validate --browser {browser} --verbose')
                                result2 = os.system(f'python ztml.py "{input_filename}" --skip_unix_newline --caps {caps} --mtf {mtf} --bitdepth {bitdepth} --bin2txt {bin2txt} {element_id_or_raw_arg} --validate --browser {browser} --verbose > {output_stream}')
