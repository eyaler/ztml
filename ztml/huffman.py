...
j=0
TEXT_NAME=''
while(j<BITARRAY_NAME.length){
c=''
k=-1
while(!((m=d[++k]?.[0]-parseInt(c,2))>=0)){
c+=BITARRAY_NAME[j]&1
j+=4}
TEXT_NAME+=s[d[k][1]+m]}
...
c+='0'.repeat(parseInt(s[j],36))
d[c]=s[j+1]
c=(parseInt(c,2)+1).toString(2).padStart(c.length,0)}
for(j=0,c=TEXT_NAME='';j<BITARRAY_NAME.length;j+=4){
c+=BITARRAY_NAME[j]&1
c in d&&(TEXT_NAME+=d[c],c='')}
