            ...
            for bin2txt in bin2txt_encodings:
                for caps in caps_modes:
                    for mtf in mtf_variants:
                        i += 1
                        print(f'{i}/{len(browsers) * len(input_encodings) * len(ztml.bin2txt_encodings) * len(caps_modes) * len(mtf_variants)} browser={browser} input_enc={encoding} bin2txt={bin2txt} caps={caps} mtf={mtf}')
                        input_filename = os.path.join(temp_folder, f'ztml_test_file_{encoding}_{bin2txt}_{caps}_{mtf}.txt')
                        output_filename = os.path.join(temp_folder, f'ztml_test_file_{encoding}_{bin2txt}_{caps}_{mtf}.html')
                        output_stream = os.path.join(temp_folder, f'ztml_test_stream_{encoding}_{bin2txt}_{caps}_{mtf}.html')
                        with open(input_filename, 'wb') as f:
                            f.write(webify.safe_encode(text, encoding))
                        if encoding == 'utf8':
                            out1, result1 = ztml.ztml(text, caps=caps, mtf=mtf, bin2txt=bin2txt, validate=True, browser=b, verbose=True)
                            out2, result2 = ztml.ztml(text, output_filename, caps=caps, mtf=mtf, bin2txt=bin2txt, validate=True, browser=b, verbose=True)
                        ...
                        result1 = os.system(f'python ztml.py "{input_filename}" "{output_filename}" --caps {caps} --mtf {mtf} --bin2txt {bin2txt} --validate --browser {browser} --verbose')
                        result2 = os.system(f'python ztml.py "{input_filename}" --caps {caps} --mtf {mtf} --bin2txt {bin2txt} --validate --browser {browser} --verbose > {output_stream}')
